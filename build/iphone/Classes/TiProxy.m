...
//Rare exceptions to indicate a bug in the _radarfoodtruck code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_radarfoodtruck = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_radarfoodtruck]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.radarfoodtruck.module"
