...
NSString * RadarFoodTruck$ModuleRequireFormat = @"(function(exports){"
...
@implementation RadarFoodTruckObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_radarfoodtruck);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create RadarFoodTruck global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "RadarFoodTruck" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_radarfoodtruck = [[RadarFoodTruckObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_radarfoodtruck];
	
	NSString *_radarfoodtruckNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _radarfoodtruckNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_radarfoodtruck valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:RadarFoodTruck$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.radarfoodtruck.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.radarfoodtruck.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
